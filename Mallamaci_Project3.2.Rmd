...
mydata <- read.csv("credit_card_applications.csv")   # lets you browse + select the file
head(mydata)
summary(mydata)
...
### 0. Identify correct churn column
# Replace "Churn" below if your column name is different (check with names(mydata))

### 1. Select numeric LOF columns
num_cols <- c("A2","A3","A4","A5","A6","A7","A10","A11","A12","A13","A14")

### 2. Build a clean dataset containing ONLY numeric columns + churn
df <- data.frame(mydata[, num_cols], Churn = mydata$Class)

### 3. Keep only complete rows
df <- df[complete.cases(df), ]

### 4. Extract cleaned columns
mydata_numeric <- df[, num_cols]
category <- df$Churn

### 5. Check lengths BEFORE LOF
print(length(category))
print(nrow(mydata_numeric))

### 6. Compute LOF with dbscan
library(dbscan)
lof_scores <- dbscan::lof(mydata_numeric, k = 100)

### 7. Check lengths match
print(length(category))
print(length(lof_scores))
